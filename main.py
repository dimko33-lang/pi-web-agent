...
def resolve_context():
    if is_local_request():
        ...
        if s and get_session(s):
            return {"ok": True, "admin": True, "alias": "local", "target_session": s}
    ...
    if alias == ADMIN_ALIAS:
        ...
        if not get_session(target):
            target = "private"
        return {"ok": True, "admin": True, "alias": alias, "target_session": target}
    target = ALIAS_MAP[alias]
    if not get_session(target):
        return {"ok": False}
    return {"ok": True, "admin": False, "alias": alias, "target_session": target}
...
# === ВЕСЬ ОСТАЛЬНОЙ КОД (оставлен без изменений) ===
@app.get("/healthz")
def healthz():
    return jsonify({"ok": True, "sessions": len(list_sessions()), "aliases": len(ALIAS_MAP), "admin_alias_exists": bool(ADMIN_ALIAS)})
...
def sessions():
    ...
    items = list_sessions()
    return jsonify({"success": True, "sessions": [s["name"] for s in items], "public_aliases": {k: v for k, v in ALIAS_MAP.items() if v in PUBLIC_SESSIONS}})

# ... (все остальные эндпоинты /chat, /events, admin и т.д. — они остались как в твоём оригинальном файле) ...
