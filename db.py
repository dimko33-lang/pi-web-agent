...
def get_or_create_session(name: str, provider: str = "groq", model: str = "llama-3.1-8b-instant") -> Dict[str, Any]:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute(
            "SELECT id, name, provider, model, created_at FROM sessions WHERE name = ?",
            (name,)
        )
        row = cur.fetchone()
        if row:
...
def get_session_by_id(session_id: int) -> Optional[Dict[str, Any]]:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute(
            "SELECT id, name, provider, model, created_at FROM sessions WHERE id = ?",
            (session_id,)
        )
        row = cur.fetchone()
        if row:
...
def list_sessions() -> List[Dict[str, Any]]:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute(
            "SELECT id, name, provider, model, created_at FROM sessions ORDER BY name"
        )
        return [
            {
                "id": r[0],
...
def clear_redo_snapshots(session_id: int):
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            "DELETE FROM redo_snapshots WHERE session_id = ?",
            (session_id,)
        )
        conn.commit()
...
def undo_last_snapshot(session_id: int) -> bool:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute(
            "SELECT id, html FROM snapshots WHERE session_id = ? ORDER BY id DESC LIMIT 1",
            (session_id,)
        )
        row = cur.fetchone()
        if not row:
...
def redo_last_snapshot(session_id: int) -> bool:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute(
            "SELECT id, html FROM redo_snapshots WHERE session_id = ? ORDER BY id DESC LIMIT 1",
            (session_id,)
        )
        row = cur.fetchone()
        if not row:
        ...
        conn.execute(
            "DELETE FROM redo_snapshots WHERE id = ?",
            (redo_id,)
        )
        conn.commit()
        return True
...
def get_history(session_id: int, limit: int = 200) -> List[Dict]:
    with sqlite3.connect(DB_PATH) as conn:
        cur = conn.execute("""
            SELECT id, role, message, provider, model, timestamp
            FROM (
                SELECT id, role, message, provider, model, timestamp
                FROM messages
                WHERE session_id = ?
